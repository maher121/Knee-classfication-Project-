...
def classwise_test_replacement(data, train_size=6000, ratio=0.2, random_state=None):
    """
    Replace a percentage of test samples with training samples
    from the same class (last column is label).

    Parameters:
    -----------
    data : np.ndarray
        Full dataset (last column = class label)
    train_size : int
        Number of rows considered as training data
    ratio : float
        Percentage of test data to replace (e.g., 0.2 for 20%)
    ...
    --------
    new_data : np.ndarray
        Dataset after class-wise replacement
    """
    ...
    for c in classes:
        # فهارس الكلاس في الاختبار
        test_idx_c = np.where(test_labels == c)[0]
        n_test_c = len(test_idx_c)

        # عدد الصفوف المطلوب استبدالها
        k = int(ratio * n_test_c)

        if k == 0:
            continue

        # فهارس الكلاس في التدريب
        train_idx_c = np.where(train_labels == c)[0]

        if len(train_idx_c) < k:
            print(f"Warning: Not enough training samples for class {c}")
            continue

        # اختيار عشوائي
        selected_test = np.random.choice(test_idx_c, k, replace=False)
        selected_train = np.random.choice(train_idx_c, k, replace=False)

        # تنفيذ الاستبدال
        test[selected_test] = train[selected_train]

    # إعادة الدمج
    new_data = np.vstack((train, test))
